...
def plot_hydrophobicity(values, window_size, output_prefix):
    """Create and save hydrophobicity plot.
    
    Args:
        values: List of hydrophobicity values
    ...
    plt.ylabel('Hydrophobicity (Kyte-Doolittle Scale)', fontsize=12)
    plt.title(f'Protein Hydrophobicity Profile (Window Size: {window_size})', fontsize=14)
    plt.grid(True, alpha=0.3)
    
    filename = f"./{output_prefix}_plot_window{window_size}.png"
...
if __name__ == '__main__':
    # FIXED: Validate command-line arguments
    if len(sys.argv) < 2:
    ...
    # Generate plots for multiple window sizes
    for window_size in [5, 7, 9]:
        hydro_values = compute_hydrophobicity(residues, window_size)
        print(f"Window {window_size}: hydrophobicity range {min(hydro_values):.2f} to {max(hydro_values):.2f}")
        plot_hydrophobicity(hydro_values, window_size, OUTPUT_PREFIX)
